...
def create_substitution_table(original, key_file='pswd.txt'):
    if ' ' not in original:
    ...
    try:
        rand_num,_,_,_= read_pswd('pswd.txt')
        ...
        # Fisher-Yates洗牌算法
        print("正在执行Fisher-Yates洗牌算法...")
        chars = list(original)
        for i in tqdm(range(len(chars) - 1, 0, -1), desc="洗牌进度", unit="步"):
            if not num_pool:
...
if __name__ == "__main__":
    ...
    char_file = './de/char/char.txt'
    if not os.path.exists(char_file):
        original = generate_ascii_charset(char_file)
        print("已生成新的ASCII字符集文件")
    else:
        print("正在读取ASCII字符集文件...")
    ...
    # 保存替换表
    print("正在保存替换表...")
    with open('./de/char/substitution.txt', 'w', encoding='utf-8') as f:
    ...
    # 打印对比示例
    print("\n[前20字符对比]")
    print(f"原始: {original[:20]}")
    print(f"替换: {substitution[:20]}")
